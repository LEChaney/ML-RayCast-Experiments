...
from keras.callbacks import LearningRateScheduler, History, ModelCheckpoint, EarlyStopping, ReduceLROnPlateau, Callback
...
BETA = 0#0.01                 #regularisation coefficient
IMAGE_ROWS = 96
IMAGE_COLS = 96
ZOOM = 2
NUM_CROPS = 1
...
LEARNING_RATE_RAY = 1e-6
LEARNING_RATE_ACTION = 1e-4
LOSS_CLIPPING = 0.2
NUM_RAYS = 5
NUM_RAY_ACTIONS = NUM_RAYS
RAY_START_VEL = 10
...
episode_localizer_input =  np.zeros((0, IMAGE_ROWS, IMAGE_COLS, 3))
episode_pred_location = np.zeros((0, 2), dtype=np.float32)

now = datetime.now().strftime("%Y%m%d-%H%M%S")
summary_writer = tf.summary.FileWriter("logs/ray_pretrained/" + now, tf.get_default_graph())
...
def build_MobileNetV2_model(trainable=False):
	input_tensor = Input(shape=(IMAGE_ROWS, IMAGE_COLS, 3)) 
	model = MobileNetV2(input_tensor=input_tensor, weights='imagenet', input_shape=(IMAGE_ROWS,IMAGE_COLS,3), include_top=False)
...
def build_ray_model():
	...
	model.compile(loss = logloss(A, NUM_RAY_ACTIONS, BETA), optimizer = optimizer) #ppo_loss(A,O, NUM_RAY_ACTIONS, BETA), optimizer = optimizer)
	return model
...
def build_action_model():
	...
	model.compile(loss = logloss(A, NUM_ACTIONS, BETA), optimizer = optimizer) #ppo_loss(A,O, NUM_ACTIONS, BETA), optimizer = optimizer)
	return model
...
def preprocess_localizer_input(img):
	img = skimage.transform.resize(img,(IMAGE_ROWS,IMAGE_COLS))
	if img.min() != img.max(): # Prevent NaNs
...
localizer_model = build_MobileNetV2_model()
localizer_model.load_weights("saved_models/model_80000_train_images_20000_val_images_1_epochs.h5")
...
def runprocess(thread_id, s_t, s_r_t, localizer_input):
	global T
	global a_t
	global ray_model
	global action_model
	...
	localizer_store_input = np.zeros((0, IMAGE_ROWS, IMAGE_COLS, 3))
	...
	pred_store_localizer = np.zeros((0, 2))
	critic_store = np.zeros((0, 1))
	critic_store_action = np.zeros((0, 1))
	localizer_input = localizer_input.reshape(1, localizer_input.shape[0], localizer_input.shape[1], localizer_input.shape[2])
	...
	while t-t_start < T_MAX and terminal == False:
		...
		with graph.as_default():
			...
			localizer_out = localizer_model.predict([localizer_input])[0]
		...
		localizer_input = preprocess_localizer_input(x_t)
		...
		localizer_out = np.reshape(localizer_out, (1,-1))
		...
		pred_store_localizer = np.append(pred_store_localizer, localizer_out, axis=0)
		...
		ray_state_starts = ray_states[thread_id]['starts'] #(ray_states[thread_id]['starts'] / current_frame.shape[0:2] * 2) - 1
		...
		#note localizer_input is updated earlier in this function
	...
	return s_r_t, s_t, state_store_ray, state_store_action, action_store_ray, action_store, pred_store_ray, pred_store_action, r_store, critic_store, localizer_input, localizer_store_input, pred_store_localizer 
...
class actorthread(threading.Thread):
	def __init__(self,thread_id, s_t, s_r_t, localizer_input):
		threading.Thread.__init__(self)
		self.thread_id = thread_id
		self.next_state = s_t
		self.next_ray_state = s_r_t
		self.next_localizer_input = localizer_input
	...
	def run(self):
		...
		global episode_localizer_input
		global episode_pred_location
		...
		self.next_ray_state, self.next_state, state_store_ray, state_store_action, action_store_ray, action_store, pred_store_ray, pred_store_action, r_store, critic_store, self.next_localizer_input, localizer_store_input, pred_store_localizer = runprocess(self.thread_id, self.next_state, self.next_ray_state, self.next_localizer_input)
		self.next_localizer_input = self.next_localizer_input.reshape(self.next_localizer_input.shape[1], self.next_localizer_input.shape[2], self.next_localizer_input.shape[3])
		...
		episode_localizer_input = np.append(episode_localizer_input, localizer_store_input, axis = 0)
		episode_pred_location = np.append(episode_pred_location, pred_store_localizer, axis = 0)
		threadLock.release()

states = np.zeros((0, IMAGE_ROWS, IMAGE_COLS, IMAGE_CHANNELS))
states_ray = np.zeros((0, (NUM_RAYS * 3 + 2) * TIME_SLICES))
input_localizers = np.zeros((0, IMAGE_ROWS, IMAGE_COLS, 3))
...
for i in range(0, len(game_state)):
	...
	localizer_input = preprocess_localizer_input(image)
	input_localizers = np.append(input_localizers, localizer_input, axis=0)
...
while True:	
	...
	threads = []
	for i in range(0,THREADS):
		threads.append(actorthread(i, states[i], states_ray[i], input_localizers[i]))
	...
	for i in range(0,THREADS):
		...
		localizer_input = threads[i].next_localizer_input
	...
	# history_localizer = localizer_model.fit([episode_localizer_input],[episode_pred_location], callbacks = callbacks_list_action, epochs = EPISODE + EPOCHS, batch_size = BATCH_SIZE, initial_epoch = EPISODE)

	episode_r = np.empty((0, 1), dtype=np.float32)
	episode_state_ray = np.zeros((0, IMAGE_ROWS, IMAGE_COLS, IMAGE_CHANNELS))
	episode_localizer_input = np.zeros((0, IMAGE_ROWS, IMAGE_COLS, 3))
	episode_pred_location = np.empty((0, 2), dtype=np.float32)
	episode_state_action = np.zeros((0, (NUM_RAYS * 3 + 2) * TIME_SLICES))
	episode_action_ray = np.empty((0, NUM_RAY_ACTIONS), dtype=np.float32)
	episode_action = np.empty((0, NUM_ACTIONS), dtype=np.float32)
	episode_pred_ray = np.empty((0, NUM_RAY_ACTIONS * 2), dtype=np.float32)
	...
	f.write("Update: " + str(EPISODE) + ", Reward_mean: " + str(e_mean) + ", Ray_Loss: " + str(history_ray.history['loss'][-1]) + ", Action_Loss: " + str(history_action.history['loss'][-1]) + ", Critic_Loss: " + str(history_critic.history['loss'][-1]) + "\n") #", Localizer_Loss: " + str(history_localizer.history['loss'][-1]) + "\n")
	f.close()
	print("Update: " + str(EPISODE) + ", Reward_mean: " + str(e_mean) + ", Ray_Loss: " + str(history_ray.history['loss'][-1]) + ", Action_Loss: " + str(history_action.history['loss'][-1]) + ", Critic_Loss: " + str(history_critic.history['loss'][-1]))# + ", Localizer_Loss: " + str(history_localizer.history['loss'][-1]))
	...
	if EPISODE % (20 * EPOCHS) == 0: 
		...
		# localizer_model.save("saved_models/localizer_model_update" + str(EPISODE))
	EPISODE += EPOCHS
